...
/**
 * 1) 이 컨트롤러는 Question관련 컨트롤러이다. 2) 비즈니스 로직은 QuestionService에 작성되어있다. 3)
 * Question의 Move(Page 이동), Create(생성), Read(읽기), Update(갱신), Delete(삭제) 기능이 있다.
 * 
 * @author leejunyeon
 */
@CrossOrigin(origins = "*")
...
public class QuestionController {
	...
	public String addQuestion(QuestionVO qstn, HttpSession session) {
		...
		// 태그 등록
		qstn.getRelatedTag().parallelStream()
				.forEach(tag -> qstnSvc.insertTag(new TagVO(qstn.getQuestionNum(), qstn.getUserNum(), tag)));
	...
	/***
	 * GET 방식으로 질문 페이지에 접근하는데 사용된다. DB에서 major리스트를 가져와서, questionForm.jsp에
	 * ${majorList}으로 전달한다.
	 * 
	 * @param model
	 *            major리스트를 ${majorList}로 사용
	...
	public String modifyQuestion_POST(QuestionVO qstn, HttpSession session, Model model) {
		...
		// 다시 등록
		qstn.getRelatedTag().parallelStream()
				.forEach(tags -> qstnSvc.insertTag(new TagVO(qstn.getQuestionNum(), qstn.getUserNum(), tags)));
		// 기존 알람 삭제 후
	...
	/**
	 * 실시간 답변 또는 동영상 녹화 답변에 사용된다. 이 페이지의 form action"addReply"으로 해둬야 답변이 DB에 올라갈듯?
	 * 
	 * @param questionNum
	 *            답변할 페이지에 출력시킬 질문의 내용
	 * @return 실시간 답변페이지
	 */
